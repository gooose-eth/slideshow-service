      ...
      <header class="tree-item__header">
        <nav class="tree-item__fold">
          <button
            ...
            <Icon :name="item.fold ? 'chevron-down' : 'minus'"/>
          </button>
        </nav>
...
function onSubmitGroup(res)
{
  ...
  {
    captureError(['/slideshow/preference/data/manage/index.vue', 'onSubmitGroup()'], 'error', e.message)
    alert('처리에 문제가 생겼습니다.')
  }
}
...
function onSubmitSlide(res)
{
  ...
  {
    captureError(['/slideshow/preference/data/manage/index.vue', 'onSubmitSlide()'], 'error', e.message)
    alert('처리에 문제가 생겼습니다.')
  }
}
</script>
