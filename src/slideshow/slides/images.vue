...
import { currentStore, dataStore, preferenceStore } from '../../store/slideshow.js'
import { setAreaTrue } from '../../libs/slideshow.js'
...
const props = defineProps({
  movePos: { type: [ Number,undefined ], default: undefined },
})
...
const containerStyle = computed(() => {
  ...
  {
    result[`--active-column`] = !isNaN(state.nextKey) ? state.nextKey : state.active
...
// methods
async function play(n, userAnimationType = undefined)
{
  // set temp active
  _active = n
  // play motion
  ...
  switch (type)
  {
    ...
    case 'horizontal':
      ...
      {
        if (state.active === 0 && _active >= data.slides.length - 1)
        {
          state.nextKey = -1
        }
        else if (state.active >= data.slides.length - 1 && _active === 0)
        {
